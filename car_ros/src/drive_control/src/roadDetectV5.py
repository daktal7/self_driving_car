...
class road_image:
    ...
    def getClosestLine(lines):
        ...
        for i in range(len(lines)):
            cur = lines[i]
            if max(cur[0,road_image.LY1],cur[0,road_image.LY2])>curMax:
                curInd = i
                curMax = max(cur[0,road_image.LY1],cur[0,road_image.LY2])
        return [lines[curInd]]
    ...
    def selectLine(lines):
        #return lines
        return road_image.getClosestLine(lines)
    ...
    def detectIntersection(im, toleranceDeg, prevLine, prevLineSearchTolerance):
        ...
        if prevLine is not None:
            boxL = max(min(prevLine[0,road_image.LY1], prevLine[0,road_image.LY2]) - prevLineSearchTolerance, 0)
            boxU = min(max(prevLine[0,road_image.LY1], prevLine[0,road_image.LY2]) + prevLineSearchTolerance, len(im[:, 0]))
            verts = np.array([[0, boxL], [0, boxU], [len(im[0, :]), boxU], [len(im[0, :]), boxL]])
            im_masked = road_image.region_of_interest(im, verts)
            ...
            if resLines is not None:
                return road_image.selectLine(resLines)
        ...
        lines = cv2.HoughLinesP(im, rho=6, theta=np.pi / 60, threshold=300, lines=np.array([]), minLineLength=100,
                                maxLineGap=400)
        return road_image.selectLine(road_image.getGoodLines(lines, toleranceDeg))
